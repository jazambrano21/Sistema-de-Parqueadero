...
export class TicketsService {
  ...
  async create(createTicketDto: CreateTicketDto, context?: any): Promise<Ticket> {
    ...
    const tenantId = context?.user?.tenantId ?? null;
    ...
    // 4. Validar que no haya ticket activo para esa placa (dentro del mismo tenant)
    await this.validarTicketActivo(createTicketDto.placa, tenantId);

    // 5. Crear y guardar el ticket con tenantId
    ...
    // 7. Invalidar caché de listas segmentada por tenant
    const suffix = tenantId ? `:${tenantId}` : '';
    await this.cacheService.del(`tickets:all${suffix}`);
    await this.cacheService.del(`tickets:activos${suffix}`);
    ...
    this.logger.log(`Ticket creado con ID: ${ticketGuardado.id} para la placa ${createTicketDto.placa} [tenant: ${tenantId}]`);
    return ticketGuardado;
  }

  // ─────────────────────────────────────────────
  // FIND ALL — cacheable, filtrado por tenant
  // ─────────────────────────────────────────────
  async findAll(tenantId?: string | null): Promise<Ticket[]> {
    const cacheKey = tenantId ? `tickets:all:${tenantId}` : 'tickets:all';
    ...
    await this.cacheService.set(cacheKey, tickets, 60);
    return tickets;
  }

  // ─────────────────────────────────────────────
  // FIND ONE — cacheable por ID, con validación de tenant
  // ─────────────────────────────────────────────
  async findOne(id: string, tenantId?: string | null): Promise<Ticket> {
    const cacheKey = `ticket:${id}`;
    const cached = await this.cacheService.get<Ticket>(cacheKey);
    if (cached) {
      // Validar que el ticket cacheado pertenezca al tenant del solicitante
  ...
  // ─────────────────────────────────────────────
  // FIND ACTIVOS — cacheable, filtrado por tenant
  // ─────────────────────────────────────────────
  async findActivos(tenantId?: string | null): Promise<Ticket[]> {
    const cacheKey = tenantId ? `tickets:activos:${tenantId}` : 'tickets:activos';
    const cached = await this.cacheService.get<Ticket[]>(cacheKey);
    if (cached) {
  ...
  async cerrarticket(id: string, updateTicketDto: UpdateTicketDto, context?: any): Promise<Ticket> {
    ...
    // Actualizar caché del ticket individual e invalidar listas segmentadas por tenant
    ...
    this.logger.log(`Ticket con ID: ${id} cerrado. Horas: ${horas}, Total: $${closeTicket.valorRecaudado} [tenant: ${tenantId}]`);
    return closeTicket;
  }
  ...
  private async validarTicketActivo(placa: string, tenantId?: string | null): Promise<void> {
    const where: any = { placa, activo: true };
    if (tenantId) where.tenantId = tenantId;
    const ticketActivo = await this.ticketRepository.findOne({ where });
    if (ticketActivo) {
